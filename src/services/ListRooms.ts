...
const fetchListElements = async (sectionId: string): Promise<ProcessedElement[]> => {
    const { data } = await axios.get<ListElementsResponse>(`https://intranet.bakhrushinmuseum.ru/rest/1/e3stpev74xjsn8m2/lists.element.get.json?IBLOCK_TYPE_ID=lists&IBLOCK_ID=80&SECTION_ID=${sectionId}`);
